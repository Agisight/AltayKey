...
class EditorViewController: UIViewController {
    ...
    @IBAction func displayHelp(_ sender: Any) {
        let alert = UIAlertController(title: "Что тестировать можно", message: "Удаление текста, вставка из буфера, копирование в буфер.  А также можете через зажатие глобуса настроить клавиатуру", preferredStyle: .alert)
    ...
    @objc func pasteText(_ sender: UIBarButtonItem) {
        if textView.text.isEmpty {
          textView.text = UIPasteboard.general.string
            ...
            let alert = UIAlertController(title: "Вставка текста", message: "Текущий текст будет заменен", preferredStyle: .actionSheet)
            alert.addAction(UIAlertAction(title: "Отмена", style: .default, handler: nil))
            alert.addAction(UIAlertAction(title: "Заменить", style: .destructive, handler: { [weak self] _ in
