...
struct Correction {
    ...
    var description: String {
        "Correction[word: \(word), isPreferred: \(isPreferred)]"
    }
}
...
struct CorrectionSet {
    ...
    static var empty: CorrectionSet {
        CorrectionSet(correction1: nil, correction2: nil, correction3: nil)
    }

    var preferredCorrection: Correction? {
        if correction1?.isPreferred ?? false {
            return correction1
        }
        if correction2?.isPreferred ?? false {
            return correction2
        }
        return nil
    }
