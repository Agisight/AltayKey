...
final class KeyboardState {
  ...
  public static var shiftState: Key.State = .off
  public static var altState: Key.State = .off
  ...
  // MARK: Modifiers
    ...
    private func tapShift() {
        if isDoubleTap(kind: .shift) {
            ...
            Self.shiftState.toggle()
        }
        displayDelegate?.shiftStateChanged(newState: Self.shiftState)
    }
  ...
  private func readAutoCapitalization() {
    if Self.shiftState == .locked { return }
    if modifierTouch != nil { return }
    if autoCapitalizer.shouldCapitalize() != Self.shiftState.isActive {
        Self.shiftState.toggle()
        displayDelegate?.shiftStateChanged(newState: Self.shiftState)
    }
  }
  ...
  private func getCurrentLetter() -> String {
    ...
    default:
      ...
      let key = keySet.key(at: keyCoordinate)
        return key.set.letter(forShiftState: Self.shiftState, andAltState: Self.altState)
    }
  }
  ...
  private func launchSubSymbolSelection() {
    ...
    let keyCoordinate = KeyLocator.calculateKeyCoordinate(for: writingTouch.currentCoordinate)
    let key = keySet.key(at: keyCoordinate)
    if key.set.subLetters(forShiftState: Self.shiftState, andAltState: Self.altState).count <= 1 { return }
    ...
    currentSubLetter = key.set.letter(forShiftState: Self.shiftState, andAltState: Self.altState)
    displayDelegate?.launchSubLetterSelection(for: key, shiftState: Self.shiftState, altState: Self.altState)
  }
  ...
  private func moveSubLetterSelection(to keypadCoordinate: KeypadCoordinate) {
    ...
    let mainLetter = originKey.set.letter(forShiftState: Self.shiftState, andAltState: Self.altState)
    let subLetters = originKey.set.subLetters(forShiftState: Self.shiftState, andAltState: Self.altState)
  ...
  private func tapDelete() {
    let shouldUppercase = (Self.shiftState == .locked) ? true : autoCapitalizer.shouldCapitalizeAfterDeletion()
    actionDelegate?.deleteBackward()
    if shouldUppercase && !Self.shiftState.isActive {
        Self.shiftState = .on
        displayDelegate?.shiftStateChanged(newState: Self.shiftState)
    } else {
      readAutoCapitalization()
